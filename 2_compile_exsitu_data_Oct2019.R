...
all_data5 <- all_data4 %>% filter(all_data4$genus_new %in% unique(species_list$genus))
  nrow(all_data5) #2396

# keep only rows from target genus and species
all_data5$genus_species <- paste(all_data5$genus_new,all_data5$species_new); unique(all_data5$genus_species)
all_data6 <- all_data5 %>% filter(all_data5$genus_species %in% unique(species_list$genus_species))
  nrow(all_data6) #1500
...
sort(unique(all_data7$infra_name_new))

# standardize infraspecific rank names
all_data7$infra_rank_new  <- mgsub(all_data7$infra_rank_new, c("v."), "var.")
all_data7$infra_rank_new  <- mgsub(all_data7$infra_rank_new, c("ssp."), "subsp.")
all_data7$infra_rank_new  <- mgsub(all_data7$infra_rank_new, c("forma"), "f.")
  unique(all_data7$infra_rank_new)

# write file
#write.csv(all_data7, "exsitu_working/exsitu_compiled_infraStandardized.csv")
...
### GET RID OF FOR LOOP ??
  for (i in 1:nrow(all_data7)){
    if(is.na(all_data7$infra_name_new[i])){
      all_data7$sp_full_name[i] <- paste(all_data7$genus_new[i],all_data7$species_new[i],sep=" ")
    } else {
      all_data7$sp_full_name[i] <- paste(all_data7$genus_new[i],all_data7$species_new[i],all_data7$infra_rank_new[i],all_data7$infra_name_new[i],sep=" ")
    }
  }; sort(unique(all_data7$sp_full_name)); nrow(all_data7) #35335

# join to species list
  all_data8 <- subset(all_data7, select = -c(extra1,extra2,extra3,extra4,extra5,extra6))
  all_data8 <- plyr::join(all_data8, species_list, type = "left", match = "first")
  # only keep matches
  all_data9 <- all_data8[which(!is.na(all_data8$our_accepted_name)),]
