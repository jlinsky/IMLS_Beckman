#SOURCE: https://github.com/EcoForecast/EF_Activities

## Exercise 3: Tools for big data ##
#========================================================

# The objective of today's exercise is to provide a quick introduction to some common tools for dealing with big data. For each tool we are just using the most basic syntax and you are encouraged to go back and read the help for each at a later date. This exercise also focuses on "general purpose"
# tools. There are a multitude of R libraries available for accessing specific data sources and web services. A quick summary of some of these is available at http://cran.r-project.org/web/views/# # WebTechnologies.html. In addition, a Google search on many of the tools and topics covered in Chapters
...
#In the previous exercises we loaded data into R using functions like read.csv. However, it is also possible to read data into R directly off the web by passing a web address to the file name. For smaller files that are quick to load this approach can ensure that the script is always
#operating with the most up-to-date version of a data file.
...
## **Question 1:**
#Using the US Forest Service's Forest Inventory and Analysis (FIA) data set, plot the rank vs log(abundance) curve for tree seedling counts from Rhode Island.
#Data is available at https://apps.fs.usda.gov/fia/datamart/CSV/RI_SEEDLING.csv and the relevant columns are TREECOUNT (raw seedling counts) and SPCD (species codes).
...
#Often the data that we want to use from the web has been formatted in HTML for human-readability rather than in tab- or comma-delimited files for inport and export.
#The process of extracting data from webpages has been dubbed **scraping**. For these sorts of more complex problems we can use the RCurl library to grab HTML or XML structured content directly off the web, and then use the XML library to parse the markup into more standard R data objects.
#In the example below we grab data on the status of all the files that make up the FIA in order to look for files that have been updated after a certain date.
...
#In this section I want to introduce another command-line utility, wget, which can be used to pull files and content off the web, and to demonstrate how netCDF can be used in R.
#For this example we will be using data from the WLEF eddy-covariance tower located in northern Wisconsin.
#Unlike most flux towers, WLEF is a "tall-tower" -- it's actually a 440m TV antenna -- which means that it integrates over a much larger footprint than most towers.
#Indeed, the tower is instrumented at multiple heights. First, let's use wget to grab the data off the web.
#A few notes:
#1) wget could be used from command line rather than as a system command;
...
#To start, lets look at the CO2 flux data, NEE_co2, which we see is stored in a matrix that has dimensions of [level2,time], where here level2 refers to the different measurements heights.
#If we want to grab this data and the vectors describing the dimensions we can do this as:
...
#Finally, we can plot the data at the different heights.
#Since this flux data is recorded hourly the raw data is a bit of a cloud, therefore we use the function `filter` to impose a 24 hour moving window, which is indicated in the function as a vector of 24 weights, each given an equal weight of 1/24.
...
## **Question 4:**
#Similar to how we can point read.csv to the URL of a text file, you can open and manipulate netCDF files on remote servers if those servers support THREDDS/OpenDAP. Furthermore, these utilities let you grab just the part of the file that you need rather than the file in it's entirety.
#Using this approach, download and plot the air temperature data for Boston for 2004 that's located on the ORNL DAAC server `http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/#mstmip_driver_global_hd_climate_tair_2004_v1.nc4`.
#The underlying file is quite large so make sure to grab just the subset you need. To do so you'll need to first grab the lat, lon, and time variables to find _which_ grid cell to grab for lat and lon and how many values to grab from time (i.e. _length_).


### Using APIs
#----------

#In addition to data that is directly downloadable, and that which is scraped, there are a number of places on the web where data is available though interactive, code-based webservices called Application Programming Interfaces (APIs).
#In this example we will access the NASA MODIS API, using a pre-existing R package called MODISTools, as a demonstration of one of the many dataset-specific R packages.
...
#`grep` is a handy little _command prompt_ function that returns lines from a file that match a search string.
#I continue to use this 'old school' utility on a daily basis to help manage code and data because this simple little search continues to be able to perform actions that elude newer search software:

#- `grep` looks within files, but is able to search across file and recursively within a directory structure.
#I use this constantly to follow variables or functions through complex code. For ex, if I wanted to find uses of the term _fia_ in my current directory and all subdirectories I could type
#grep -ir "fia" .

#here the -i means ignore case when searching, the -r means to search recursively through subdirectories, and the `.` means to start from the current directory. Used in this way grep can help you quickly find your way through new and complex code, iteratively hopping through the code from one
#search to another. It is also extremely helpful in debugging, when a program returns a cryptic error message and you want to find _where_ in the code that message was generated.
...
#or I could be more specific an just look for specific letters, e.g. fia[fds] would match fiaf, fiad, and fias. A full description of regular expressions is beyond the scope of this tutorial, and RegExp statements for matching complex patterns can quickly become cryptic, so following up on this
#further is left to the reader.
