...
sid <- sid %>% group_by(taxon_name,dataset) %>%
  summarise(
    ...
    SID_taxon_name_notes = paste(SID_taxon_name_notes, collapse = '; '))
nrow(sid) # 24713
# recode exceptional status column when multiple or typo
sid <- sid %>% mutate(SID_exceptional_status = recode(SID_exceptional_status,
                      "Recalcitrant; Recalcitrant?" = "Recalcitrant?",
                      "Orthodox; Orthodox?" = "Orthodox?",
                      "Orthodox; Orthodox p" = "Orthodox p",
                      "Intermediate`" = "Intermediate"))
unique(sid$SID_exceptional_status)
...
# fix up inconsistencies in text
  ...
  sid$SID_storcond <- str_squish(sid$SID_storcond)
  # remove space before % sign and after dash; other small fixes
  sid$SID_storcond <- gsub(" %", "% ",sid$SID_storcond)
  sid$SID_storcond <- gsub("- ", "-",sid$SID_storcond)
  sid$SID_storcond <- gsub("afew", "a few",sid$SID_storcond)
  sid$SID_storcond <- gsub("chaange", "change",sid$SID_storcond)
  # replace unusual characters in degree celsius text
  sid$SID_storcond <- mgsub(sid$SID_storcond,
    c("¡","¼","â"), "")
  ...
  sid$SID_storcond_orig <- sid$SID_storcond
  # make everything lowercase for easier matching
  sid$SID_storcond <- str_to_lower(sid$SID_storcond)
...
# separate by ")" and keep the character in the output
sid_unique <- separate_rows(sid_unique, SID_storcond, sep = "(?<=\\))")
nrow(sid_unique) #44573

# create subset for practice
sid_ex <- sid_unique#[sample(nrow(sid_unique), 300), ]

## LOOK AT TEMPERATURE & TIME THRESHOLD RECORDS BEFORE DOING VIABILITY ANALYSIS
...
flag <- or(
  ...
  "moist storage",
  "dorman",
  or("viability ","seeds ") %R%
    optional("can be ") %R%
    or("maintained ","retained ") %R%
    or("after ","best at ","following ","for ") %R%
    one_or_more(DGT) %R% or(" months "," year "," years ") %R%
      or("in commercial storage conditi",
      "storage at -15c","at -15c","hermetic storage at -15c",
      "storage at -17c",
      "hermetic storage at -18c","in hermetic air-dry storage at -18c",
      "in storage at -20c")
)
...
temp_capture <- str_match(sid_ex$SID_storcond,pattern=temperature)
  ...
  temp_capture[grepl("room|herbarium|temperate|laboratory|cool|ambient|sub-zero",
    temp_capture[,1]),1] <- "ambient (>-15 celsius)"
  temp_capture[grepl("commercial|preferred",
    temp_capture[,1]),1] <- "preferred (-15 to -25 celsius)"
  temp_capture_val <- as.numeric(temp_capture[,2])
  temp_capture[temp_capture_val> -15 &
    temp_capture_val<100,1] <- "ambient (>-15 celsius)"
  temp_capture[temp_capture_val<= -15 &
    temp_capture_val>= -25,1] <- "preferred (-15 to -25 celsius)"
  temp_capture[temp_capture_val>100 |
    temp_capture_val < -25,1] <- "unknown"
...
# filter by temp & time cutoffs before looking at viability
#sid_ex_filtered <- sid_ex %>%
#  filter(SID_exceptional_status == "Orthodox" |
#    SID_exceptional_status == "Orthodox p" |
#    SID_exceptional_status == "Orthodox?" |
#    SID_exceptional_status == "Uncertain") %>%
#  filter(temperature_c <= -15 & temperature_c >= -25) %>%
#  filter(time_years <= 20)
#nrow(sid_ex_filtered) #19386

# look at most common data formats
#nrow(sid_ex_filtered[which(grepl(
#  "% viability following drying to mc's in equilibrium with 15% rh and freezing for",
#  sid_ex_filtered$SID_storcond)),]) #16646
#nrow(sid_ex_filtered[which(grepl(
#  "long-term storage under ipgri preferred conditions",
#  sid_ex_filtered$SID_storcond)),]) #2001

# can look at potential matches
sid_ex[which(grepl("soil seed bank",sid_ex$SID_storcond)),]$SID_storcond
...
percent_abs <-
  ...
  optional("%") %R% optional(or("-"," to "," and ")) %R%
  zero_or_more(DGT) %R% optional(or(DOT,"-")) %R% one_or_more(DGT) %R% "%"
viability_abs <- or(
    # phrases with percent
  percent_abs %R% or(" germination"," viability"," germinate"," survive"),
  or("viability ","germination ") %R% percent_abs
)
percent_diff <-
  zero_or_more(DGT) %R% optional(DOT) %R% zero_or_more(DGT) %R%
  optional("%") %R%
  optional(or("-"," to "," and "," before storage to ",", ")) %R%
  zero_or_more(DGT) %R% optional(or(DOT,"-")) %R% one_or_more(DGT) %R% "%"
viability_diff <- or(
    # phrases with percent
  percent_diff %R% " viability lost",
  or("viability reduced ","viability is reduced ","germination change ") %R%
    optional(connector) %R% percent_diff,
...
# view matches
  #str_view_all(sid_ex$SID_storcond, pattern = viability_diff)
  #str_view_all(sid_ex$SID_storcond, pattern = viability_abs)
  ...
  str_view_all(sid_ex$SID_storcond, pattern = flag, match = T)

# extract matches
## DIFFERENCE IN VIABILITY
  # capture viability text, including percents
    #### FLAG ROWS WITH MORE THAN ONE MATCH??###
viability_capture_d <- str_match(sid_ex$SID_storcond,pattern=viability_diff)
  sid_ex$viability_verbatim_difference <- viability_capture_d[,1]
  sort(unique(sid_ex$viability_verbatim_difference))
  viability_capture_d <- mgsub(viability_capture_d,
    c("half-life","p50","halved"),"50%")
  capture_values <-
    capture(zero_or_more(DGT) %R% optional(DOT) %R% one_or_more(DGT)) %R%
    optional("%") %R% or("%","-"," to "," and "," before storage to ","±") %R%
    optional(capture(zero_or_more(DGT) %R% optional(DOT) %R% one_or_more(DGT)))
  viability_capture_val <- str_match(viability_capture_d,pattern=capture_values)
    # calculate loss
  viability_capture_val[!is.na(viability_capture_val[,3]),2] <-
    round(as.numeric(viability_capture_val[!is.na(viability_capture_val[,3]),2])-
    as.numeric(viability_capture_val[!is.na(viability_capture_val[,3]),3]),0)
  sid_ex$viability_percent_difference <- as.numeric(viability_capture_val[,2])
  sort(unique(sid_ex$viability_percent_difference))
## ABSOLUTE VIABILITY
viability_capture_a <- str_match(sid_ex$SID_storcond,pattern=viability_abs)
  sid_ex$viability_verbatim_absolute <- viability_capture_a[,1]
  sort(unique(sid_ex$viability_verbatim_absolute))
  viability_capture_val <- str_match(viability_capture_a,pattern=capture_values)
    # if second values is given, use that value
  viability_capture_val[!is.na(viability_capture_val[,3]),2] <-
    viability_capture_val[!is.na(viability_capture_val[,3]),3]
  sid_ex$viability_percent_absolute <- round(
    as.numeric(viability_capture_val[,2]),0)
  sort(unique(sid_ex$viability_percent_absolute))
...
as.data.frame(head(sid_ex))
# condense dup taxa that were created at beginning; put storcond back together
sid_ex_unq <- sid_ex %>%
  arrange(SID_exceptional_status,desc(viability_percent_difference,flag)) %>%
  group_by(taxon_name) %>%
  mutate(
    #SID_storcond = paste(SID_storcond, collapse = '; '),
...
sid_ex_unq$SID_exceptional_status <- factor(sid_ex_unq$SID_exceptional_status,
  ...
  ggplot(sid_ex_unq,
    aes(x=time_years, y=viability_percent_difference, color=SID_storbehav)) +
    geom_point(alpha = 0.5) +
    xlim(0, 150) +
    theme(text = element_text(size = 20))
    ggsave("SID keyword analysis/viabilityDifferenceVStime.jpg",width=7,height=7)
  ggplot(sid_ex_unq,
    aes(x=time_years, y=viability_percent_absolute, color=SID_storbehav)) +
    geom_point(alpha = 0.5) +
    xlim(0, 150) +
    theme(text = element_text(size = 20))
    ggsave("SID keyword analysis/viabilityAbsoluteVStime.jpg",width=7,height=7)
  temp_subset <- sid_ex_unq %>%
    filter(temperature_bin == "preferred (-15 to -25 celsius)")
  ggplot(temp_subset,
    aes(x=time_years, y=viability_percent_absolute, color=SID_storbehav)) +
    geom_point(alpha = 0.5) +
    xlim(0, 30) +
    ...
    # viability as x-axis
  ggplot(sid_ex_unq,
    aes(x=viability_percent_absolute, y=moisture_content, color=SID_storbehav)) +
    geom_point(alpha = 0.5)
    ggsave("SID keyword analysis/moistureVSviability.pdf",width=7,height=7)
  ggplot(sid_ex_unq,
    aes(x=viability_percent_absolute, y=relative_humidity, color=SID_storbehav)) +
    geom_point(alpha = 0.5)
    ggsave("SID keyword analysis/humidityVSviability.pdf",width=7,height=7)
  ggplot(sid_ex_unq,
    aes(x=temperature_c, y=viability_percent_difference, color=SID_storbehav)) +
    geom_point(alpha = 0.5) +
    xlim(-25, 30)
    ggsave("SID keyword analysis/viabilityDifferenceVStemp.pdf",width=7,height=7)
  ## temperature bins
  ggplot(sid_ex_unq,
    aes(x=time_years, y=viability_percent_absolute, color=SID_storbehav)) +
    geom_point(alpha = 0.5) +
...
# filter by specific cutoffs
sid_ex_difference <- sid_ex_unq %>%
  #filter(SID_exceptional_status == "Orthodox" | SID_exceptional_status == "Orthodox p" |
  #  SID_exceptional_status == "Orthodox?" | SID_exceptional_status == "Uncertain") %>%
  filter(viability_percent_difference >= 25) %>%
  filter(temperature_c <= -15 & temperature_c >= -25) %>%
  filter(time_years <= 20) #time_years >= 1
nrow(sid_ex_difference) #558; 3546; 10
sid_ex_absolute <- sid_ex_unq %>%
  #filter(SID_exceptional_status == "Orthodox" | SID_exceptional_status == "Orthodox p" |
  #  SID_exceptional_status == "Orthodox?" | SID_exceptional_status == "Uncertain") %>%
  filter(viability_percent_absolute <= 85) %>%
  filter(temperature_c <= -15 & temperature_c >= -25) %>%
  filter(time_years <= 20) #time_years >= 1
nrow(sid_ex_absolute) #558; 3546; 10
...
table(all_data_join$taxonomicStatus)
  # Accepted 350007 ; heterotypicSynonym 1944 ; homotypicSynonym 251 ;
  # Synonym 415288 ; Unchecked 208397
...
# remove synonyms and unmatched species
genus_tbl2 <- genus_tbl %>%
  filter(taxonomicStatus == "Accepted") %>% # | taxonomicStatus == "Unchecked"
  group_by(genus) %>%
  summarize(number_Exceptional = sum(number_Exceptional),
            number_NonExceptional = sum(number_NonExceptional),
            number_Uncertain = sum(number_Uncertain),
            number_NoData = sum(number_NoData))
genus_tbl2$total_Species <- rowSums(genus_tbl2[,c(2:5)])
genus_tbl2 <- genus_tbl2[,c(1:4,6)]
genus_tbl2$percent_Exceptional <- round(genus_tbl2$number_Exceptional/genus_tbl2$total_Species*100,2)
genus_tbl2$percent_Assessed <- round(rowSums(genus_tbl2[,c(2:4)])/genus_tbl2$total_Species*100,2)
head(as.data.frame(genus_tbl2),n=40)

# write file
write.csv(genus_tbl2,"genus_level_table_acceptedOnly.csv",row.names=F)

# remove spp. records before moving on
all_data4 <- all_data4[which(!grepl("spp.",all_data4$genus_species)),]
...
nrow(threats) #92628
# combine duplicates
threats <- setdiff(threats,threats[duplicated(threats$genus_species),])
str(threats) #76910

# match threat data to exceptional list
threat_join <- threats %>% dplyr::select(genus_species,threat_status,
  redlistCategory)
all_data5 <- left_join(all_data4,threat_join)
all_data5$threat_status[which(is.na(all_data5$threat_status))] <- "Unknown"
head(all_data5)

###############################################################################
# 9. Query Plantminer for taxonomic information and match to IPNI also
###############################################################################

# only look at Exceptional and Uncertain records
no_orthodox <- all_data5 %>%
  filter(exceptional_status != "Non-exceptional")

## Plantminer
# list of names to match
taxa_names <- no_orthodox$genus_species; length(taxa_names)
# query Plantminer; can take a while if more than a few hundred names
plantminer_names <- plantminer(taxa_names)
str(plantminer_names)
# standardize column names for joining
plantminer_names2 <- plantminer_names %>%
  rename(genus_species = original.search,
         tax_id = id,
         tax_authorship = authorship,
         tax_source = source,
         tax_note = note,
         tax_name_matched = name,
         tax_status = taxonomic.status.in.tpl
       ) %>%
  dplyr::select(genus_species,tax_source,tax_authorship,#tax_id,
    tax_note,tax_name_matched,tax_status)
nrow(plantminer_names2[which(plantminer_names2$tax_source==""),]) #198 not found
plantminer_names2 <- plantminer_names2 %>%
  filter(tax_name_matched != "")
plantminer_names2$genus_species <- str_squish(plantminer_names2$genus_species)

## IPNI
# get data
ipni_names2 <- ipni_all %>%
  arrange(desc(version)) %>%
  rename(genus_species = full_name_without_family_and_authors,
         tax_id = id,
         tax_authorship = authors,
         tax_name_matched = full_name_without_authors
       ) %>%
  dplyr::select(genus_species,tax_authorship,#tax_id,
    tax_name_matched)
ipni_names2$tax_source <- "IPNI"
ipni_names2$tax_status <- "Accepted"
ipni_names2 <- distinct(ipni_names2,genus_species,.keep_all=T)

## join
tax_info <- rbind.fill(plantminer_names2,ipni_names2); nrow(tax_info)
tax_info <- distinct(tax_info,genus_species,.keep_all=T); nrow(tax_info)
# match to list
all_data5 <- left_join(all_data5,tax_info)
  # unmatched rows
nrow(all_data5[which(is.na(all_data5$tax_source)),]) #731
nrow(all_data5[which(is.na(all_data5$tax_source) &
                     all_data5$exceptional_status != "Non-exceptional"),]) #65
...
all_data6 <- all_data5 %>%
  dplyr::select("family","genus_species","taxon_name","exceptional_status",
    ...
    "tax_source","tax_authorship","tax_note","tax_name_matched","tax_status"
  )
# write file
write.csv(all_data6,"exceptional_species_list_8_5.csv",row.names=F)

# look at results by family
fams <- all_data6 %>%
  mutate(exceptional_status = recode(exceptional_status,
    "Uncertain*" = "Uncertain")) %>%
  count(exceptional_status,family) %>%
  arrange(desc(n))
top_fams <- reshape(fams,idvar="family",timevar="exceptional_status",
  direction="wide")
top_fams$total <- rowSums(top_fams[,c(2:4)])
top_fams <- top_fams %>% arrange(desc(total))
head(top_fams,n=30)
top_fams$percent_Exceptional <- round(top_fams$n.Exceptional/top_fams$total*100,2)
top_fams <- top_fams %>% arrange(desc(percent_Exceptional))
head(top_fams,n=30)

#ggplot(bar_fams, aes(x = n, y = reorder(family,n))) +
#  geom_col(aes(fill = exceptional_status),
#    position = position_stack(reverse = T)) +
#  theme(legend.position = "top", axis.text = element_text(size = (8))) +
#  ggtitle("Species with exceptionality data, by family") +
#  labs(y = "Family",x = "Number of species")
#  ggsave("Exceptionality_by_Family.pdf",
#    width=20,height=12)
...
# Find ALL threatened congeners
# select genera that are exceptional, not based on unavailable seed
exceptional <- all_data6 %>%
  filter(exceptional_status == "Exceptional" &
         standard_justification != "Seeds unavailable") %>%
  separate("genus_species","genus",sep=" ",extra="warn",remove=F)
excep_genera <- unique(exceptional$genus)
# filter threated taxa by exceptional genera
threatened <- threats %>% filter(threat_status == "Threatened") %>%
  arrange(genus_species)
threat_cong <- threatened[which(threatened$genus %in% excep_genera),]
threat_cong$redlistCategory[which(threat_cong$redlistCategory == "NA")] <- "Not Evaluated"
nrow(threat_cong) #31017
head(threat_cong)
threat_cong <- threat_cong[,c(1,3)]
# write file
write.csv(threat_cong,"threatened_congeners_table_all.csv",row.names=F)

# Find only WFO ACCEPTED congeners
# match WFO species to threat records
threatened <- threatened[,-2]
nrow(threatened) #66469
wfo_threats <- right_join(wfo_species,threatened)
# remove unmatched threat records
wfo_threats <- wfo_threats %>%
  filter(!is.na(family))
nrow(wfo_threats) #60639
# keep only accepted records
wfo_threats_acc <- wfo_threats %>%
  filter(taxonomicStatus == "Accepted") %>%
  arrange(genus_species)
nrow(wfo_threats_acc) #48234
# filter threated taxa by exceptional genera
threat_cong_acc <- wfo_threats_acc[which(wfo_threats_acc$genus %in% excep_genera),]
threat_cong_acc$redlistCategory[which(threat_cong_acc$redlistCategory == "NA")] <- "Not Evaluated"
nrow(threat_cong_acc) #22916
head(threat_cong_acc)
threat_cong_acc <- threat_cong_acc[,c(3,5)]
# write file
write.csv(threat_cong_acc,"threatened_congeners_table_WFOaccepted.csv",
  row.names=F)
