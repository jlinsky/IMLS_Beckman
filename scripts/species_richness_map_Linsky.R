...
hist(map_countries@data$Freq,breaks=90,xlim=c(0,150),ylim=c(0,20))
	#bins <- c(0,1,5,10,15,20,40,70,100,150,Inf)
	bins <- c(0,1,5,10,20,30,40,50,150)
	labels <- c("0","1-4","5-9","10-19","20-29","30-39","40-49","50+")
	# create color palette
	#display.brewer.all()
	palette_country <- colorBin(palette = "PuRd", bins = bins,
		domain = map_countries@data$Freq, reverse = F, na.color = "white")
	# create map
	legend <- paste0("Number of native","<br/>","magnolia species")
	map_richness <- map.countries(map_countries,palette_country,legend,labels)
	map_richness
	
	##Error in hist.default(map_countries@data$Freq, breaks = 90, xlim = c(0,  : 
	 ##character(0)
  ...
  # save map
  htmlwidgets::saveWidget(map_richness, "GlobalMagnoliaRichness_leaflet_map.html")
...
# THREATENED species
	...
	# create map
	legend <- paste0("Number of native,","<br/>","threatened Magnolia species")
	map_richness_th <- map.countries(map_countries_th,palette_country,legend,labels)
	map_richness_th
  # save map
  htmlwidgets::saveWidget(map_richness_th, "GlobalThreatenedMagnoliaRichness_leaflet_map.html")

# ENDEMIC species
	# create color bins and labels
	hist(map_countries_en@data$Freq,breaks=19,xlim=c(0,80),ylim=c(0,20))
	bins <- c(0,1,2,3,5,10,15,40,Inf)
	labels <- c("0","1","2","3-4","5-9","10-14","15-39","40+")
	# create color palette
	#display.brewer.all()
	palette_country <- colorBin(palette = "PuRd", bins = bins,
		domain = map_countries_en@data$Freq, reverse = F, na.color = "white")
	# create map
	legend <- paste0("Number of endemic","<br/>","Magnolia species")
	map_richness_en <- map.countries(map_countries_en,palette_country,legend,labels)
	map_richness_en
  # save map
  htmlwidgets::saveWidget(map_richness_en, "GlobalEndemicMagnoliaRichness_leaflet_map.html")
