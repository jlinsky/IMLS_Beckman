...
acc_list <- list.files(path="./Desktop/work/MortonBRAHMS_IMLSgenera_Dec2020_Accessions",
...
# rename necessary columns and create others
...
##
### PLANTS DATA TABLE (has number of individuals and garden location; i.e., if propagation)
##

# read in all Excel plants workbooks in data folder
plt_list <- list.files(path="./Desktop/work/MortonBRAHMS_IMLSgenera_Dec2020_Plants",
  pattern=".xlsx",full.names=TRUE)
# read in each excel file in path list to create list of dataframes
...
# make everything not in propagation be "collections"
...
nrow(all_data)
# remove accessions with no match in plant table (no longer active or in natural areas)
...
# write file
write.csv(all_data2,
  "./Desktop/work/Morton_IMLS_accessions_Dec2020.csv",row.names = F)
