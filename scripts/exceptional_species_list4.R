...
viability_diff <- or(
  ...
  or("viability reduced ","germination change ") %R% optional(connector) %R%
    percent_diff,
      # other phrases to match (no percent)
      # all/most lost
  "complete loss in viability",
  "viabilty is completely lost",
  "total loss of viability",
  "none survive",
      # half lost
  "half-life",
  "p50",
...
# filter by time & temp cutoffs before looking at viability
sid_ex <- sid_ex %>%
  ...
  filter(temperature_c <= -15 & temperature_c >= -22) %>%
  filter(time_years <= 20) #time_years >= 1 &
nrow(sid_ex) #558; 3546
...
viability_capture_d <- str_match(sid_ex$SID_storcond,pattern=viability_diff)
  ...
  viability_capture_d <- mgsub(viability_capture_d,
    c("complete","few seeds","maximum","total","none"),"0%")
  capture_values <-
    capture(zero_or_more(DGT) %R% optional(DOT) %R% one_or_more(DGT)) %R%
    optional("%") %R% or("%","-"," to "," and "," before storage to ","±") %R%
    optional(capture(zero_or_more(DGT) %R% optional(or(DOT,"-")) %R%
    one_or_more(DGT)))
...
#      viability_capture),2]),2)
  #sid_ex$viability_percent_d <- as.numeric(viability_capture_val[,2])
  #sort(unique(sid_ex$viability_percent_d))
## ABSOLUTE VIABILITY
viability_capture_a <- str_match(sid_ex$SID_storcond,pattern=viability_abs)
  ...
  #viability_capture_a <- mgsub(viability_capture_a,
  #  c("reduced"),"80%")
  viability_capture_a <- mgsub(viability_capture_a,
  ...
  viability_capture_val[!is.na(viability_capture_val[,3]),2] <-
    viability_capture_val[!is.na(viability_capture_val[,3]),3]
  sid_ex$viability_percent_absolute <- as.numeric(viability_capture_val[,2])
